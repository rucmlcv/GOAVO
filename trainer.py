...
class Trainer:
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            self.att, outputs = outputs

            if self.opt.gc == "True":
                if self.epoch > 15:
                    g_loss = self.discriminate_g(inputs, outputs)
                    losses["g_loss"] = g_loss
                    losses["loss"] += 0.01 * g_loss

                g_loss = self.discriminate_g(inputs, outputs)
                losses["g_loss"] = g_loss
                losses["loss"] += g_loss

                d_loss = self.discriminate_d(inputs, outputs)
                losses["d_loss"] = d_loss
                losses["loss"] += losses["d_loss"]
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs = self.models["depth"](features[0], 0,
                                           self.opt.use_attention_depth)
        else:
            features = self.models["encoder"](
                inputs["color_aug", 0, 0])  # torch.Size([12, 64, 96, 320])
    ...
    def discriminate_g(self, inputs, outputs):
        """Discriminator of GAN
        """
        adversarial_loss = torch.nn.BCELoss()
        imgs_shape = inputs[("color", 0, 0)].shape
        Tensor = torch.FloatTensor

        valid = Variable(Tensor(imgs_shape[0]).to(self.device).fill_(1.0),
                         requires_grad=False)
        ...
        for scale in self.opt.scales:
            ...
            # pred_m1 = outputs[("color", -1, scale)].detach()
            pred_m1 = outputs[("color", -1, scale)]

            if torch.isnan(pred_m1.min()) or torch.isnan(pred_m1.max()):
                continue

            if pred_m1.max().data > 1.0:
                pred_m1 = torch.clamp(pred_m1, 0.0, 1.0)

            validity = self.models["discriminator"](pred_m1)
            loss_m1 = adversarial_loss(validity, valid)

            pred_p1 = outputs[("color", 1, scale)]
            if torch.isnan(pred_p1.min()) or torch.isnan(pred_p1.max()):
                continue
            if pred_p1.max().data > 1.0:
                pred_p1 = torch.clamp(pred_p1, 0.0, 1.0)
            pred_p1 = torch.clamp(pred_p1, 0.0, 1.0)
            validity = self.models["discriminator"](pred_p1)

            loss_p1 = adversarial_loss(validity, valid)

            loss = (loss_m1 + loss_p1) / 2
            total_loss += loss
    ...
    def discriminate_d(self, inputs, outputs):
        """Discriminator of GAN
        """
        adversarial_loss = torch.nn.BCELoss()
        imgs_shape = inputs[("color", 0, 0)].shape
        ...
        valid = Variable(Tensor(imgs_shape[0]).to(self.device).fill_(1.0),
                         requires_grad=False)
        fake = Variable(Tensor(imgs_shape[0]).to(self.device).fill_(0.0),
                        requires_grad=False)
        ...
        for scale in self.opt.scales:
            ...
            if pred_m1.max().data > 1.0:
                pred_m1 = torch.clamp(pred_m1, 0.0, 1.0)
            validity = self.models["discriminator"](pred_m1)
            loss_m1 = adversarial_loss(validity, fake)

            pred_p1 = outputs[("color", 1, scale)].detach()
            if torch.isnan(pred_p1.min()) or torch.isnan(pred_p1.max()):
                continue
            if pred_p1.max().data > 1.0:
                pred_p1 = torch.clamp(pred_p1, 0.0, 1.0)
            validity = self.models["discriminator"](pred_p1)
            loss_p1 = adversarial_loss(validity, fake)

            pred_t = inputs[("color", 0, source_scale)].detach()
            if torch.isnan(pred_t.min()) or torch.isnan(pred_t.max()):
                continue
            if pred_t.max().data > 1.0:
                pred_t = torch.clamp(pred_t, 0.0, 1.0)
            validity = self.models["discriminator"](pred_t)
            loss_t = adversarial_loss(validity, valid)

            loss = (loss_m1 + loss_p1 + loss_t) / 3
            total_loss += loss
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    axisangle, translation = self.models["pose"](
                        pose_inputs, self.opt.use_attention_pose)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            elif self.opt.use_attention_depth:
                # use the our attention predicted mask
                mask = self.att[-1]
                if not self.opt.v1_multiscale:
                    mask = F.interpolate(mask,
                                         [self.opt.height, self.opt.width],
        ...
        for j in range(min(
            ...
            for s in self.opt.scales:
                ...
                writer.add_image("disp_{}/{}".format(s, j),
                                 normalize_image(outputs[("disp", 0, s)][j]),
                                 self.step)
                ...
                if self.opt.predictive_mask:
                    for f_idx, frame_id in enumerate(self.opt.frame_ids[1:]):
                        writer.add_image(
                            ...
                            outputs["predictive_mask"][("disp", 0,
                                                        s)][j, f_idx][None,
                                                                      ...],
                            self.step)
