...
class ConvBlock_sig(nn.Module):
    """Layer to perform a convolution followed by ELU
    """
    def __init__(self, in_channels, out_channels):
        super(ConvBlock_sig, self).__init__()
...
class Conv1x1_mat(nn.Module):
    """Layer to pad and convolve input
    """
    def __init__(self, in_channels, out_channels, use_refl=True):
        super(Conv1x1_mat, self).__init__()
...
class ConvBlock_1x1(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels):
        super(ConvBlock_1x1, self).__init__()
...
class DeConv(nn.Module):
    """ Layer to Deconv input
    """
    def __init__(self, in_channels, out_channels):
        super(DeConv, self).__init__()

        self.deconv = nn.ConvTranspose2d(int(in_channels),
                                         int(out_channels),
                                         3,
                                         stride=2,
                                         padding=1,
...
class Project3D(nn.Module):
    ...
    def forward(self, points, K, T):
        ...
        pix_coords = cam_points[:, :2, :] / (cam_points[:, 2, :].unsqueeze(1) +
                                             self.eps)
        pix_coords = pix_coords.view(self.batch_size, 2, self.height,
                                     self.width)
...
def get_smooth_loss(disp, img):
    ...
    grad_img_x = torch.mean(torch.abs(img[:, :, :, :-1] - img[:, :, :, 1:]),
                            1,
                            keepdim=True)
    grad_img_y = torch.mean(torch.abs(img[:, :, :-1, :] - img[:, :, 1:, :]),
                            1,
                            keepdim=True)
